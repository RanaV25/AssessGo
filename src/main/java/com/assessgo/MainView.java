...
import com.assessgo.frontend.components.navigation.bar.TabBar;
import com.assessgo.frontend.components.navigation.drawer.NaviDrawer;
import com.assessgo.frontend.components.navigation.drawer.NaviItem;
import com.assessgo.frontend.components.navigation.drawer.NaviMenu;
import com.assessgo.frontend.util.UIUtils;
import com.assessgo.frontend.util.css.FlexDirection;
import com.assessgo.frontend.util.css.Overflow;

import com.assessgo.frontend.views.assessment.AddAssessmentView;
import com.assessgo.frontend.views.assessment.AssessmentView;
import com.assessgo.frontend.views.assessment.UsersAssessmentView;
import com.assessgo.frontend.views.dashboard.DashboardView;
import com.assessgo.frontend.views.requirement.RequirementView;
import com.assessgo.frontend.views.user.UserView;
...
@PWA(name = "AssessGo", shortName = "AssessGo", iconPath = "images/logo-18.png", backgroundColor = "#233348", themeColor = "#233348")
...
public class MainView extends FlexBoxLayout
    ...
    private void initStructure() {
        ...
        UIUtils.setTheme(Lumo.DARK, naviDrawer);
    ...
    private void initNaviItems() {
        ...
        menu.addNaviItem(VaadinIcon.HOME, "Dashboard", DashboardView.class);
        menu.addNaviItem(VaadinIcon.USER, "Users", UserView.class);

        NaviItem learning = menu.addNaviItem(VaadinIcon.OPEN_BOOK,"Learning",
                null);

        if(SecurityUtils.isUserHasRole(Role.ADMIN) || SecurityUtils.isUserHasRole(Role.SUPER_ADMIN)) {
            menu.addNaviItem(learning,"Assessment", AssessmentView.class);
        }

        menu.addNaviItem(learning,"Assessment View", UsersAssessmentView.class);

        menu.addNaviItem(VaadinIcon.FOLDER_OPEN, "Requirement", RequirementView.class);
    }
    ...
    private void initHeadersAndFooters() {
        ...
        appBar = new AppBar("");
        add(appBar.createChangePasswordDialog());
    ...
    public void showRouterLayoutContent(HasElement content) {
        ...
        if(content instanceof AddAssessmentView) {
            appBar.setTitle("Add Assessment");
            appBar.setNaviMode(AppBar.NaviMode.CONTEXTUAL);
            appBar.getContextIcon().addClickListener(
                    e -> UI.getCurrent().navigate(AssessmentView.class));
        }

        if(content instanceof UsersAssessmentView) {
            appBar.setTitle("View Assessments");
            appBar.setNaviMode(AppBar.NaviMode.CONTEXTUAL);
            appBar.getContextIcon().addClickListener(
                    e -> UI.getCurrent().navigate(AssessmentView.class));
        }
